                            ...
                            if IsControlJustPressed(0, 38) then
                                if CurrentCops >= Config.MinimumStoreRobberyPolice then
                                    ...
                                    if not copsCalled then
                                        ...
                                        local cameraId = Config.Safes[currentSafe].camId exports['ps-dispatch']:StoreRobbery(cameraId)                                    end
                                else
            ...
            if CurrentCops >= Config.MinimumStoreRobberyPolice then
                if usingAdvanced then
                    ...
                    if not copsCalled then
                        ...
                        local cameraId = Config.Registers[currentRegister].camId exports['ps-dispatch']:StoreRobbery(cameraId)                        copsCalled = true
                    end
                else
                    lockpick(true)
                    currentRegister = k
...
-- Added event trigger for evidence drop
    TriggerServerEvent("evidence:server:CreateLockTampering", pos)
